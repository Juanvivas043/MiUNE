...
class CmcBytes_Filtros {
   ...
   public function addCustom($nombre,$fill,$id=null){
    ...
    public function generateQueriesF($tablas, $filter = null, $ejecucion = NULL, $elemento = NULL) {
        $queryarray = Array();
        $html = '';
        $js = '';
        
        // Lógica de ejecución inicial
        if ($ejecucion == 1) {
            $label = array_keys($tablas);
            $html .= $this->genertaBoxes($label);
    
            // Recorrer las tablas y sus elementos
            foreach ($tablas as $key => $tab) {
                foreach ($tab as $keys => $t) {
                    switch ($keys) {
                        case 0:
                            ...
                            break;
                        case 1:
                            $where = ($t != NULL) ? $this->generateReserved($t, 'WHERE', $filtersarray) : '';
                            break;
                        case 2:
                            $columnas = $this->generateReserved($t, 'SELECT', NULL);
                            break;
                ...
                // Recorrer los resultados de la consulta para rellenar las opciones de los selects
                foreach ($queryresult as $mikey => $res) {
                    if ($mikey == 0) {
                        $selected2 = Array($key => $res[$llaves[0]]);
                        $html .= "<option value=''>------------------</option>";
                ...
                // Asignar las opciones generadas a cada select
                $js .= '$("#' . $key . '").html("' . $html . '");';
                $html = ''; // Limpiar el HTML después de cada set de opciones
                $filtersarray = array_merge($selected2, $queryarray);
                $queryarray = $filtersarray;
            }
        } else {
            // Lógica recursiva para manejar filtros después de la ejecución inicial
            $mod_keys = array_keys($filter);
            $cont2 = 0;
            $done = false;
            foreach ($tablas as $key => $tab) {
                if ($cont2 > $cont) {
                    foreach ($tab as $keys => $t) {
                        switch ($keys) {
                            ...
                            case 1:
                                if ($t != NULL) {
                                    $where = ($done == true) ? $this->generateReserved($t, 'WHERE', $filtersarray) : $this->generateReserved($t, 'WHERE', $filter);
                                } else {
                                    $where = '';
                                }
                                break;
        ...
        // NUEVO: Llamada AJAX para enviar los filtros sin depender de un evento 'change'
        $js .= "
            var filtros = $('#filtro').find(':input').serialize();
            $.getJSON('$url' + 'filter/select/' + '$key' + '/filters/' + escape(filtros), 
                function(d) { 
