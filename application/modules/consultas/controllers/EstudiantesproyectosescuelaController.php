<?php
class Consultas_EstudiantesproyectosescuelaController extends Zend_Controller_Action {

  private $_title = 'Consultas \ Estudiantes Inscritos en Proyectos por Escuela';
  ...
  public function listAction() {
    if ($this->_request->isXmlHttpRequest()) {
      $this->SwapBytes_Ajax->setHeader();
      $json = array();
      $rows = $this->inscripcionespasantias->getEstudiantesProyectosEscuelas($this->_params['filters']['Periodo'], $this->_params['filters']['Sede'],$this->_params['filters']['Escuela']);
      if (!empty($rows)) {
        $table = array('class' => 'tableData',
                       'width' => '1180px');
        $columns = 
        array(
          array('column' => 'pk_inscripcionpasantia',
              'primary' => true,
              'hide' => true),
          array('name'     => '#',
               'width'    => '15px',
  ...
  public function descargarAction() {
    ...
    $report = APPLICATION_PATH . '/modules/reports/templates/Serviciocomunitario/EstudianteProyectoEscuela.jrxml'; 
