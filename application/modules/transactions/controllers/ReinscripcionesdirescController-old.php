<?php
class Transactions_ReinscripcionesdirescController extends Zend_Controller_Action {
...
/*---------*/public function indexAction() {
            ...
            public function getcedulaAction(){
...
/*----------*/public function infoestudianteAction($ci){
                    $this->SwapBytes_Ajax->setHeader(); 
                    $c = 0;
                        ...
                        if($c != 3){
                           ...
                           //Indice acumulado y del periodo
                            ...
                            if($countins[0]['count']>1){
                                ...
                                $json[] = '$("#informacion").hide()';
                                $json[] = $this->sweetalert->setBasicAlert("error","Error","El estudiante no posee materias a inscribir.");
                                $json[] .= "$('.confirm').click(function(){window.location.replace('$this->redirect')});";
                            }elseif($count > 0){

                                $json[] = '$("#informacion").hide()';
                                $json[] = $this->sweetalert->setBasicAlert("error","Error","El estudiante posee {$count} libro(s) de la bilioteca en Transito o Mora. Debe realizar la devolución para poder inscribirse.");
                                $json[] .= "$('.confirm').click(function(){window.location.replace('$this->redirect')});";
...
/*-----*/public function seccionesAction(){
            ...
            $this->usede  = $usede = $this->reinscripciones->sedeUltimoPeriodo($this->cedula);
            if($this->usede == NULL){
                $this->usede  = $usede = $this->reinscripciones->sedePrimerPeriodo($this->cedula);
            }
            $this->pens   = $this->reinscripciones->getPensumNew($this->cedula, $usede[0]['fk_periodo']);
            $this->usede  = $this->pens[0]['pk_estructura'];
            $this->cod    = $this->_getParam('cod1');
            $sem          = $this->_getParam('sem1');
            $tur          = $this->_getParam('tur1');
            if(isset($this->pens[0]['fk_pensum']) and isset($this->per) and isset($this->usede) and isset($sem) and isset($tur) and isset($this->cod)) { 
...
/*----*/public function inscribirmateriasAction(){
            ...
            foreach ($seccion as $key => $value) {
                if($value == '--'){
                    $json[] = "$('#msg').html('Alguna de las materias a inscribior no tiene una sección escogida');
                               $('#msg').show();";
                    $test = 1;
                    break;
                }
