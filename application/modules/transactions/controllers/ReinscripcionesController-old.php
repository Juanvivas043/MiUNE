...
/*---*/public function unidadescreditoestudianteAction(){
            ...
            if($credito > $ucu+ $aUcadicio){ //si hay mas unidades de credito de las que posee entonces se deshabilita el boton de finalizar
                ...
                $json[] = "$('#btnInscribir').addClass('btnUnvisible').removeClass('btnVisible').addClass('exceso');";
            }
                ...
                $json[] = "$('#msg').hide();";
                $json[] = "setTimeout(function(){ if($('#btnInscribir').hasClass('coincidencia')== false) {
                                 $('#btnInscribir').addClass('btnVisible').removeClass('btnUnvisible').attr('disabled', false).removeClass('exceso');
                             } }, 800);";
            }
            echo Zend_Json::encode($json);        
            ...
            foreach ($coincidencia as $i => $value) {
                foreach ($coincidencia as $j => $value) {
                    if (   $coincidencia[$i]['asignatura']!=$coincidencia[$j]['asignatura']&&$coincidencia[$i]['dia']==$coincidencia[$j]['dia']&&$coincidencia[$i]['hora']==$coincidencia[$j]['hora']){
                            $json[]   = "$('#btnInscribir').addClass('btnUnvisible').removeClass('btnVisible').addClass('coincidencia');";
                            break 2;
                        ...
                        $json[] = "if($('#btnInscribir').hasClass('btnUnvisible') && $('#btnInscribir').hasClass('exceso') == false) {
