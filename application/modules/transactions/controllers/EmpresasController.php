<?php
class Transactions_EmpresasController extends Zend_Controller_Action {   
 
  private $_Title             = 'Transacciones \ Lista de Empresas';
  ...
  public function init() {
    ...
    Zend_Loader::loadClass('Forms_Empresa');
    ...
    // FORM  
    $this->view->form = new Forms_Empresa();
    $this->SwapBytes_Form->set($this->view->form);
    $this->SwapBytes_Form->fillSelectBox('fk_tipopasantia',  $this->vw_tiposinstituciones->getTipos(8223)  , 'pk_atributo', 'institucion');
  ...
  public function existsAction(){
      if ($this->_request->isXmlHttpRequest()) {
          ...
          if(!empty($rif)) {
              ...
              $dataRow        = $dataRow[0];
              // Verificamos si la empresa existe, al ser asi evitamos que 
              ...
              else if(!empty($dataRow) and $dataRow['fk_tipopasantia'] == 8223) {
              ...
              if(!empty($dataRow) and $dataRow['fk_tipopasantia'] == 8223){ 
                  $dataRow['id']  = $id_row;
                  ...
                  //Si no existe pero estoy editando una empresa (cargo informacion previa)
                  if($id_row > 0){
  ...
  public function listAction() {   
    if ($this->_request->isXmlHttpRequest()) {
      ...
      $rows = $this->Instituciones->getListaEmpresas();
      ...
      $columns = array(array('column'  => 'pk_institucion',
          ...
          $HTML  = $this->SwapBytes_Html_Message->alert("No existen Empresas cargadas.");
          $json[] = $this->SwapBytes_Jquery->setHtml('tableData', $HTML);
      }
      $this->getResponse()->setBody(Zend_Json::encode($json));
    }
